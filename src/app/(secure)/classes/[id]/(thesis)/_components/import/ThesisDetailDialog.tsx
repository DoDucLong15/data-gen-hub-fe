...
import { THESIS_DETAIL_DIALOG } from '@/configs/messages.config';
...
export function ThesisDetailDialog({
  ...
  const renderFieldValue = (column: ColumnConfig, value: any) => {
    if (!value) return THESIS_DETAIL_DIALOG.NO_INFO;
      ...
      <DialogContent className="max-h-[90vh] max-w-3xl overflow-hidden rounded-lg border p-0 shadow-lg">
        <DialogHeader className="border-b bg-gradient-to-r from-blue-50 to-indigo-50 px-4 py-3">
          ...
          <div className="mt-1 flex items-center justify-between">
            ...
            >
              <X className="h-4 w-4" />
              <span className="sr-only">{THESIS_DETAIL_DIALOG.CLOSE}</span>
            </Button>
          </div>
        </DialogHeader>
            ...
            {metadataColumns.length > 0 && (
              <div className="mb-4">
                <div className="mb-2 flex items-center gap-1.5 text-gray-700">
                  ...
                  <h3 className="text-sm font-semibold">{THESIS_DETAIL_DIALOG.SECTIONS.BASIC_INFO.TITLE}</h3>
                </div>
            ...
            {regularColumns.length > 0 && (
              <div className="mb-4">
                <div className="mb-2 flex items-center gap-1.5 text-gray-700">
                  ...
                  <h3 className="text-sm font-semibold">{THESIS_DETAIL_DIALOG.SECTIONS.DETAILS.TITLE}</h3>
                </div>
            ...
            {contentColumns.length > 0 && (
              <div className="mb-4">
                <div className="mb-2 flex items-center gap-1.5 text-gray-700">
                  ...
                  <h3 className="text-sm font-semibold">{THESIS_DETAIL_DIALOG.SECTIONS.CONTENT.TITLE}</h3>
                </div>
                    ...
                    <Card key={column.accessorKey} className="overflow-hidden border border-gray-300 shadow-sm">
                      ...
                      <CardContent className="bg-white px-4 py-3">
                        <div className="text-sm leading-relaxed whitespace-pre-wrap text-gray-800">
                          {entity[column.accessorKey] || THESIS_DETAIL_DIALOG.NO_INFO}
                        </div>
                      </CardContent>
                    </Card>
                  ))}
            ...
            {config.downloadEnabled && entity.inputPath && (
              <div className="mb-4">
                <div className="mb-2 flex items-center gap-1.5 text-gray-700">
                  ...
                  <h3 className="text-sm font-semibold">{THESIS_DETAIL_DIALOG.SECTIONS.ATTACHMENTS.TITLE}</h3>
                </div>
                <div className="flex items-center justify-between rounded border border-blue-200 bg-blue-50 p-3">
                    ...
                    <div>
                      <p className="max-w-[200px] truncate text-sm font-medium text-gray-800 md:max-w-[300px]">
                        {entity.fileName ||
                          ...
                          THESIS_DETAIL_DIALOG.SECTIONS.ATTACHMENTS.FILE_LABEL}
                      </p>
                      <p className="text-xs text-gray-600">{THESIS_DETAIL_DIALOG.SECTIONS.ATTACHMENTS.DOWNLOAD_HINT}</p>
                    </div>
                  </div>
                  <Button
                    variant="secondary"
                  ...
                  >
                    <Download className="h-4 w-4" />
                    {THESIS_DETAIL_DIALOG.SECTIONS.ATTACHMENTS.DOWNLOAD_BUTTON}
                  </Button>
                </div>
              </div>
            )}
