...
import { useOneDrives } from '@/hooks/userOneDrive';
...
export const FileUploader: React.FC<FileUploaderProps> = ({ classId, isOpen, onClose, folderId, driveId }) => {
  const [files, setFiles] = useState<File[]>([]);
  const [uploadProgress, setUploadProgress] = useState(0);
  const { uploadFilesMutation: uploadFiles } = useOneDrives(classId);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await uploadFiles.mutateAsync({ files, parentId: folderId, driveId });
