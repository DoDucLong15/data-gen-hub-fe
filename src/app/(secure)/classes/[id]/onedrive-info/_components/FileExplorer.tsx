...
import { useOneDriveNavigation, useOneDrives, useOneDriveSelection } from '@/hooks/userOneDrive';
...
export const FileExplorer: React.FC<FileExplorerProps> = ({ classId }: { classId: string }) => {
  ...
  const [previewFile, setPreviewFile] = useState<TOnedriveItem | null>(null);
  const [isPreviewOpen, setIsPreviewOpen] = useState(false);

  const { fileTree, loadingFileTree: isLoading, errorFileTree: isError } = useOneDrives(classId);
  const { breadcrumbs, getCurrentFolder, navigateToFolder, navigateToBreadcrumb } = useOneDriveNavigation(
    fileTree || null,
  );

  const { selectedFiles, toggleFileSelection, clearSelection, isSelected } = useOneDriveSelection();

  const handleFilePreview = (file: TOnedriveItem) => {
    setPreviewFile(file);
