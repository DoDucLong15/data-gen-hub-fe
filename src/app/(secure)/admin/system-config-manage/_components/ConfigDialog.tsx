...
export const EditConfigDialog = ({ 
  isOpen, onClose, config, onSave, isLoading = false 
}: EditDialogProps) => {
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>
            {config 
              ? `Editing configuration with key: ${config.key}` 
...
export const DeleteConfigDialog = ({
  isOpen, onClose, configKey, onConfirm, isLoading = false
}: DeleteDialogProps) => {
  return (
    <AlertDialog open={isOpen} onOpenChange={onClose}>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            This will permanently delete the configuration with key: <strong>{configKey}</strong>.
