...
import { ACCOUNT_PAGE } from '@/configs/messages.config';
...
export default function AccountLayout() {
    ...
    return (
      <div className="container mx-auto flex min-h-[400px] items-center justify-center py-10">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mt-4">{ACCOUNT_PAGE.LOADING.MESSAGE}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="container mx-auto py-10">
        <Card className="bg-card/30 backdrop-blur-sm">
          <CardContent className="py-10">
            <div className="text-center">
              <h2 className="text-xl font-semibold">{ACCOUNT_PAGE.NOT_FOUND.TITLE}</h2>
              <p className="text-muted-foreground mt-2">{ACCOUNT_PAGE.NOT_FOUND.DESCRIPTION}</p>
              <Button
                className="bg-primary text-primary-foreground hover:bg-primary/90 mt-4 rounded-md px-4 py-2 transition-colors"
                onClick={() => router.push('/account/login')}
              >
                {ACCOUNT_PAGE.NOT_FOUND.LOGIN_BUTTON}
              </Button>
            </div>
          </CardContent>
        </Card>
  ...
  return (
    <div className="container mx-auto px-6 py-10">
      <h1 className="mb-6 text-2xl font-bold">{ACCOUNT_PAGE.TITLE}</h1>
